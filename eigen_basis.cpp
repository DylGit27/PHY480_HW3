...
/*
 Analysis: Looking at the output plots, the numerical approximations converge closer to the true solution as D increases, and the approximation becomes exact when the dimension is increased to infinity. Since b sets the width of the basis functions and the fall-off rate at large r, the optimal value of b changes based on the dimension. For all dimensions used, b=2 was closer to the true solution compared to b=3, so for at least small dimensions, b=2 is closer to the optimal value than b=3. We know the wavefunction is normalized because integrating over all values of r yields one.
 */
//******************************************************************
...
main ()
{
  ...
  ofstream out ("#3");	// open the output file
  double r = 0.1; //starting value
  double rend = 10; //end value
  double dr = .1; //step size
  out << "r" << " " << "u(r)" << endl;
   ...
   // cout << "eigenvalue " << i+1 << " = "
    ...
    //******NEW CODE********
      while (r < rend)
       ...
       for (int j = 0; j < dimension; j++)
       {
		 u+=gsl_vector_get (eigenvector_ptr, j) * ho_radial (j+1, 0, b_ho,r);
